...
export default function GeneralModal({ open, onClose, childComponent, maxWidth, title, wrapInBox = true }) {
  return (
    <Modal
      open={open}
      onClose={onClose}
    ...
    >
      ...
      >
        {/* Conditionally wrap in Box component */}
        {wrapInBox ? (
          <Box
            sx={{
          ...
          >
            <CardContent>{childComponent}</CardContent>
          </Box>
        ) : (
          <CardContent>{childComponent}</CardContent>
        )}
...
GeneralModal.propTypes = {
  ...
  wrapInBox: PropTypes.bool // Adding the new prop type
};
